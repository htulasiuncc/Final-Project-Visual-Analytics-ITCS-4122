...
#Choropleth
        ...
        choropleth_map5 = px.choropleth(avg_wind_speed_by_state, locations='State', color='Wind_Speed(mph)',
         ...
         # Choropleth map 6: Average distance of accidents by state
        avg_distance_by_state = cleaned_data.groupby('State')['Distance(mi)'].mean().reset_index()
        choropleth_map6 = px.choropleth(avg_distance_by_state, locations='State', color='Distance(mi)',
                                        locationmode='USA-states', scope='usa',
                                        title='Average Distance of Accidents by State')
        ...
        st.plotly_chart(choropleth_map6)
